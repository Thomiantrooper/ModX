...
// Define an async thunk for updating an enquiry with the manager's reply
export const replyToEnquiry = createAsyncThunk(
  "enquiry/reply-to-enquiry",
  async ({ enquiryId, managerReply }, thunkAPI) => {
    try {
      return await enquiryService.replyToEnquiry(enquiryId, managerReply);
    } catch (error) {
...
export const enquirySlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Add new cases for replyToEnquiry async thunk
      .addCase(replyToEnquiry.pending, (state) => {
        state.isLoading = true;
      })
      ...
      .addCase(replyToEnquiry.rejected, (state, action) => {
        state.isLoading = false;
        state.isError = true;
        state.isSuccess = false;
        state.message = action.error;
